...
public interface ArticlesDao {
    ...
    @Select("select * from articles where status = #{status} limit #{startIndex},30")
    ...
    //我的上传
    @Select("select * from articles where takerId = #{current} and owner is null")
    ...
    //更新物品状态
    @Update("update articles set status = #{status},owner = '' where id = #{id}")
    int cancelClaim(Long id,int status);
